/* Freestanding PE32 Direct3D 12 animated cube. */
#define COBJMACROS
...
static int run(void)
{
    ...
    cls.lpszClassName = "WineBrowserD3D12Cube";
    if (!RegisterClassA(&cls)) return 1;
    RECT bounds = {0, 0, WIDTH, HEIGHT};
    if (!AdjustWindowRect(&bounds, WS_OVERLAPPEDWINDOW, FALSE)) return 2;
    HWND window = CreateWindowExA(0, cls.lpszClassName, "WineBrowser Direct3D 12 cube",
    ...
    pso_desc.VS.pShaderBytecode = cube_vs;
    pso_desc.VS.BytecodeLength = sizeof(cube_vs);
    pso_desc.PS.pShaderBytecode = cube_ps;
    pso_desc.PS.BytecodeLength = sizeof(cube_ps);
